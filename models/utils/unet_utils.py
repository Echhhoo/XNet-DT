...
from models.utils.dropblock import DropBlock3D,DropBlock2D
...
class UnetUp3(nn.Module):#采用转置卷积进行上采样3d
...
class UnetUp3_CT(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        # print(outputs1.size())
        # print(outputs2.size())
        # print((torch.cat([outputs1, outputs2],1).shape))
        return self.conv(torch.cat([outputs1, outputs2], 1))



class Unet3_CT(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm=True):
        super(Unet3_CT, self).__init__()
        self.conv = UnetConv3(in_size, out_size, is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1))
        # self.up = nn.Sequential(*[Upsample(wavename = 'haar')])
