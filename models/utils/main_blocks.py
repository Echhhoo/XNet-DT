...
def visualize(images, x, y):
    """
    此函数可能需要根据三维数据的可视化需求进一步调整，目前先保留原逻辑，后续可能需要适配三维图像展示逻辑
    """
    c, h, w = images.shape
...
class conv_block(nn.Module):
    def __init__(self,
                 ...
                 stride=(1, 1, 1),  # 修改为三维步长
                 padding=(1, 1, 1),  # 修改为三维padding
                 dilation=(1, 1, 1),
                 norm_type='bn',
                 activation=True,
                 use_bias=True,
...
def conv_block_3D(in_features, out_features, kernel_size=(3, 3, 3), padding=(1, 1, 1), norm_type=None, activation=False):
    layers = []
    layers.append(nn.Conv3d(in_channels=in_features, out_channels=out_features, kernel_size=kernel_size, padding=padding))
    if norm_type == 'batch':
        layers.append(nn.BatchNorm3d(num_features=out_features))
    elif norm_type == 'instance':
        layers.append(nn.InstanceNorm3d(num_features=out_features))
    if activation:
...
class transpose_conv_block(nn.Module):
    def __init__(self,
                 in_features,
                 out_features,
                 kernel_size=(3, 3, 3),  # 修改为三维转置卷积核尺寸
                 stride=(2, 2, 2),  # 修改为三维步长
                 padding=(0, 0, 0),
                 out_padding=(1, 1, 1),
                 dilation=(1, 1, 1),
                 norm_type='bn',
        ...
        super().__init__()
        self.conv = nn.ConvTranspose3d(in_channels=in_features,
                                        out_channels=out_features,
                                        kernel_size=kernel_size,
                                        stride=stride,
                                        padding=padding,
...
class bn_relu(nn.Module):
    def __init__(self, features) -> None:
        super().__init__()
        self.bn = nn.BatchNorm3d(features)  # 修改为三维批归一化
        self.relu = nn.ReLU()
...
class SqueezeExciteBlock(nn.Module):
    def __init__(self, in_features, reduction: int = 16):
        super().__init__()
        self.avgpool = nn.AdaptiveAvgPool3d(1)  # 修改为三维自适应平均池化
        self.fc = nn.Sequential(
            nn.Linear(in_features, int(in_features // reduction), bias=False),
            nn.ReLU(),
            nn.Linear(int(in_features // reduction), in_features, bias=False),
            nn.Sigmoid())

    def forward(self, x):
        b, c, _, _, _ = x.size()  # 获取三维数据的维度信息
        y = self.avgpool(x).view(b, c)
        y = self.fc(y).view(b, c, 1, 1, 1)  # 调整维度以匹配三维数据
        out = x * y.expand_as(x)
        return out
...
class AttentionBlock(nn.Module):
    def __init__(self, input_encoder, input_decoder, output_dim, norm_type='bn'):
        ...
        if norm_type == 'bn':
            self.norm1 = nn.BatchNorm3d(input_encoder)  # 修改为三维批归一化
            self.norm2 = nn.BatchNorm3d(input_decoder)  # 修改为三维批归一化
            self.norm3 = nn.BatchNorm3d(output_dim)  # 修改为三维批归一化
        ...
        self.conv_encoder = nn.Sequential(
            self.norm1,
            nn.ReLU(),
            nn.Conv3d(input_encoder, output_dim, 3, padding=1),  # 修改为三维卷积
            nn.MaxPool3d(2, 2),  # 修改为三维最大池化
        )

        self.conv_decoder = nn.Sequential(
            self.norm2,
            nn.ReLU(),
            nn.Conv3d(input_decoder, output_dim, 3, padding=1),  # 修改为三维卷积
        )

        self.conv_attn = nn.Sequential(
            self.norm3,
            nn.ReLU(),
            nn.Conv3d(output_dim, 1, 1),  # 修改为三维卷积，输出通道为1用于注意力权重
        )
...
class ResConv(nn.Module):
    def __init__(self, in_features, out_features, stride=(1, 1, 1)):  # 修改为三维步长
        super().__init__()
        self.conv = nn.Sequential(
            bn_relu(in_features),
            nn.Conv3d(in_channels=in_features,
                      out_channels=out_features,
                      kernel_size=(3, 3, 3),
                      padding=(1, 1, 1),
                      stride=stride),
        ...
        self.skip = nn.Conv3d(in_channels=in_features,
                              out_channels=out_features,
                              kernel_size=(1, 1, 1),
                              padding=(0, 0, 0),
                              stride=stride)
...
class rrcnn_block(nn.Module):
    def __init__(self,
        ...
        self.norm = nn.BatchNorm3d(out_features)  # 修改为三维批归一化
        self.norm_c = nn.BatchNorm3d(out_features)  # 修改为三维批归一化
        self.relu = nn.ReLU()
...
class ASPP(nn.Module):
    def __init__(self, in_features, out_features, norm_type='bn', activation=True, rate=[1, 6, 12, 18]):
        ...
        self.out = conv_block(
            ...
            out_features=out_features,
            kernel_size=(1, 1, 1),  # 修改为三维 1x1x1 卷积核用于输出调整
            padding=(0, 0, 0),
            norm_type=norm_type,
            activation=activation
        )
...
class DoubleASPP(nn.Module):
    def __init__(self, 
        ...
        self.block1 = conv_block(
            in_features=in_features,
            out_features=out_features,
            kernel_size=(1, 1, 1),  # 修改为三维 1x1x1 卷积核
            padding=(0, 0, 0),
            norm_type=norm_type,
            activation=activation
        )
