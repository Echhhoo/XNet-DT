...
def random_rescale_intensity(image1,image2, label):
    image1 = exposure.rescale_intensity(image1)
    image2 = exposure.rescale_intensity(image2)
    return image1,image2,label


def random_equalize_hist(image1,image2, label):
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() > 0.5:
            image1,image2,label = RandomRotFlip(image1,image2, label)
        if random.random() <0.2:
            image1,image2,label = RandomRotFlip(image1,image2,label, cval=0)
        if random.random() > 0.9:
            image1,image2,label = RandomNoise(image1,image2,label)
        if random.random() > 0.33:
            image1,image2,label = nonlinear_transformation(image1,image2,label)
        elif random.random() < 0.66 and random.random() > 0.33:
            image1,image2,label = random_rescale_intensity(image1,image2,label)
        elif random.random() > 0.66:
            image1,image2,label = random_equalize_hist(image1,image2,label)
        w, h, d = image1.shape
        image1 = zoom(
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if self.with_sdf:
            ...
            return {'image1': image1,'image2': image2,'label': label, 'sdf': sdf}
        else:
...
class CreateOnehotLabel(object):
    ...
    def __call__(self, sample):
        ...
        return {'image1': image1,'image2': image2, 'label': label,'onehot_label':onehot_label}
