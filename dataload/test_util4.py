...
def test_single_case(net, image1,image2,image3, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image1.shape
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    y1,y2,y3 = net(test_patch1,test_patch2,test_patch3)
                    # ensemble
...
def test_all_case(net, base_dir, method="unet_3D", test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, test_save_path=None):
    ...
    with open(test_save_path + "/{}.txt".format(method), "a") as f:# 这里新建了一个文档但是没有写入
        for image_path in tqdm(image_list):
            ...
            # image2 = L
            # image3 = H
            prediction1,prediction2,prediction3 = test_single_case(net, image1,image2,image3,stride_xy, stride_z, patch_size, num_classes=num_classes)
            ...
            sitk.WriteImage(img_itk, test_save_path +"/{}_img.nii.gz".format(ids))
