# nohup python ASOCA_train_semi_v2_dice.py >xnetv2_dwt_semi_fold00.out &
...
if __name__ == '__main__':
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, sampled_batch_labeled in enumerate(dataloaders['train_sup']):
            scaler =torch.amp.GradScaler('cuda')
            inputs_train_unsup_1 = Variable(sampled_batch_labeled['image1'].cuda())
            inputs_train_unsup_2 = Variable(sampled_batch_labeled['image2'].cuda())
            inputs_train_unsup_3 = Variable(sampled_batch_labeled['image3'].cuda())
            optimizer.zero_grad()
            with torch.amp.autocast('cuda',enabled=True): 
                outputs_train_unsup_1, outputs_train_unsup_2 ,outputs_train_unsup_3=  model(inputs_train_unsup_1, inputs_train_unsup_2, inputs_train_unsup_3)
            ...
            inputs_train_sup_1 = Variable(sampled_batch_labeled['image1'].cuda())
            inputs_train_sup_2 = Variable(sampled_batch_labeled['image2'].cuda())
            inputs_train_sup_3 = Variable(sampled_batch_labeled['image3'].cuda())
            mask_train_sup = Variable(sampled_batch_labeled['label'].cuda())
            with torch.amp.autocast('cuda',enabled=True): 
                outputs_train_sup_1, outputs_train_sup_2 ,outputs_train_sup_3=  model(inputs_train_sup_1, inputs_train_sup_2, inputs_train_sup_3)
                torch.cuda.empty_cache()
                if i == 0:
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('xnetv2_dwt_semi_fold03.png')