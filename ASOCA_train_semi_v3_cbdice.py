# nohup python ASOCA_train_semi_v3_cbdice.py >semi_asoca_xnetv3_fold00.out &        
...
from loss.compound_cbdice_loss import DC_CE_CBDC_loss
...
from dataload.Dataset5 import BaseDataSets, RandomRotFlip, RandomCrop, ToTensor
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_dataset', default='/home/ac/datb/wch/ASOCA_DTCWT')
    parser.add_argument('--root_path', type=str,default='/home/ac/datb/wch/ASOCA_DTCWT', help='Name of Experiment')
    ...
    parser.add_argument('-n', '--network', default='xnetv3_3d_add', type=str)
    ...
    lambda_dice = 1.0
    lambda_ce = 1.0
    cbdice_loss= DC_CE_CBDC_loss({'batch_dice': args.batch_size, 'smooth': 1e-5, 'do_bg': False}, {},
                                    {'iter_': 10, 'smooth': 1e-3},
                                    weight_ce=lambda_ce, weight_dice=lambda_dice, weight_cbdice=lambda_cbdice,num_epochs=args.num_epochs)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(num_batches['train_sup']):
            ...
            inputs_train_unsup_1 = Variable(unsup_index['image1'].cuda(non_blocking=True))
            inputs_train_unsup_2 = Variable(unsup_index['image2'].cuda(non_blocking=True))
            inputs_train_unsup_3 = Variable(unsup_index['image3'].cuda(non_blocking=True))
            inputs_train_unsup_4 = Variable(unsup_index['image4'].cuda(non_blocking=True))
            
            optimizer.zero_grad()
            with torch.amp.autocast('cuda',enabled=True):  
                outputs_train_unsup_1, outputs_train_unsup_2 ,outputs_train_unsup_3= model(inputs_train_unsup_1, inputs_train_unsup_2, inputs_train_unsup_3,inputs_train_unsup_4)
                ...
                max_train_unsup3 = torch.max(outputs_train_unsup_3, dim=1)[1].long()
            ...
            # loss_train_unsup = dice_loss(outputs_train_unsup_1, max_train_unsup2[:].unsqueeze(1)) + dice_loss(outputs_train_unsup_2, max_train_unsup1[:].unsqueeze(1))+dice_loss(outputs_train_unsup_1, max_train_unsup3[:].unsqueeze(1)) + dice_loss(outputs_train_unsup_3, max_train_unsup1[:].unsqueeze(1))
                loss_train_unsup =cbdice_loss(outputs_train_unsup_1, max_train_unsup2[:].unsqueeze(1),epoch) + cbdice_loss(outputs_train_unsup_2, max_train_unsup1[:].unsqueeze(1),epoch)+cbdice_loss(outputs_train_unsup_1, max_train_unsup3[:].unsqueeze(1),epoch) + cbdice_loss(outputs_train_unsup_3, max_train_unsup1[:].unsqueeze(1),epoch)
            ...
            inputs_train_sup_1 = Variable(sup_index['image1'].cuda(non_blocking=True))
            inputs_train_sup_2 = Variable(sup_index['image2'].cuda(non_blocking=True))
            inputs_train_sup_3 = Variable(sup_index['image3'].cuda(non_blocking=True))
            inputs_train_sup_4 = Variable(sup_index['image4'].cuda(non_blocking=True))
            mask_train_sup =  Variable(sup_index['label'].cuda(non_blocking=True))
            torch.cuda.empty_cache()
            with torch.amp.autocast('cuda',enabled=True): 
                outputs_train_sup_1, outputs_train_sup_2 ,outputs_train_sup_3=  model(inputs_train_sup_1, inputs_train_sup_2, inputs_train_sup_3,inputs_train_sup_4)
                if count_iter % args.display_iter == 0:
                    if i == 0:
                        score_list_train_1 = outputs_train_sup_1
                        score_list_train_2 = outputs_train_sup_2
                        score_list_train_3 = outputs_train_sup_3
                    ...
                    # elif 0 < i <= num_batches['train_sup'] / 32:
                        ...
                        score_list_train_3 = torch.cat((score_list_train_3, outputs_train_sup_3), dim=0)
            ...
            # loss_train_sup3 =  dice_loss(outputs_train_sup_3, mask_train_sup[:].unsqueeze(1))
                ...
                loss_train_sup1 =  cbdice_loss(outputs_train_sup_1, mask_train_sup[:].unsqueeze(1),epoch)
                loss_train_sup2 =  cbdice_loss(outputs_train_sup_2, mask_train_sup[:].unsqueeze(1),epoch)
                loss_train_sup3 =  cbdice_loss(outputs_train_sup_3, mask_train_sup[:].unsqueeze(1),epoch)
        ...
        if count_iter % args.display_iter == 0:
            ...
            with torch.no_grad():
                ...
                for i, sampled_batch_labeled in enumerate(dataloaders['val']):
                    ...
                    inputs_val_4 = Variable(sampled_batch_labeled['image4'].cuda())
                    mask_val = Variable(sampled_batch_labeled['label'].cuda())
                    

                    optimizer.zero_grad()
                
                    outputs_val_1, outputs_val_2 ,outputs_val_3 = model(inputs_val_1, inputs_val_2, inputs_val_3,inputs_val_4)
                    torch.cuda.empty_cache()
                    ...
                    loss_val_sup_1 = cbdice_loss(outputs_val_1, mask_val[:].unsqueeze(1),epoch)
                    loss_val_sup_2 = cbdice_loss(outputs_val_2, mask_val[:].unsqueeze(1),epoch)
                    loss_val_sup_3 = cbdice_loss(outputs_val_3, mask_val[:].unsqueeze(1),epoch)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('semi_asoca_xnetv3_fold03.png')