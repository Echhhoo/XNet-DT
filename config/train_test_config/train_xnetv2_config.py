...
def print_val_eval_sup(num_classes, score_list_val_1,score_list_val_2,score_list_val_3, mask_list_val, print_num):
    if num_classes == 2:
        eval_list1 = evaluate(score_list_val_1, mask_list_val)
        eval_list2 = evaluate(score_list_val_2, mask_list_val)
        eval_list3 = evaluate(score_list_val_3, mask_list_val)
        print('| Val Thr 1: {:.4f}'.format(eval_list1[0]).ljust(print_num, ' '), '| Val Thr 2: {:.4f}'.format(eval_list2[0]).ljust(print_num, ' '), '| Val Thr 3: {:.4f}'.format(eval_list3[0]).ljust(print_num, ' '), '|')
        print('| Val  Jc 1: {:.4f}'.format(eval_list1[1]).ljust(print_num, ' '), '| Val  Jc 2: {:.4f}'.format(eval_list2[1]).ljust(print_num, ' '), '| Val  Jc 3: {:.4f}'.format(eval_list3[1]).ljust(print_num, ' '), '|')
        print('| Val  Dc 1: {:.4f}'.format(eval_list1[2]).ljust(print_num, ' '), '| Val  Dc 2: {:.4f}'.format(eval_list2[2]).ljust(print_num, ' '), '| Val  Dc 3: {:.4f}'.format(eval_list3[2]).ljust(print_num, ' '), '|')
        val_m_jc1 = eval_list1[1]
        val_m_jc2 = eval_list2[1]
        val_m_jc3 = eval_list3[1]
    else:
        eval_list1 = evaluate_multi(score_list_val_1, mask_list_val)
        eval_list2 = evaluate_multi(score_list_val_2, mask_list_val)
        eval_list3 = evaluate_multi(score_list_val_3, mask_list_val)
        np.set_printoptions(precision=4, suppress=True)
        print('| Val  Jc1: {}  '.format(eval_list1[0]).ljust(print_num, ' '), '| Val  Jc2: {}  '.format(eval_list2[0]).ljust(print_num, ' '), '| Val  Jc3: {}  '.format(eval_list3[0]).ljust(print_num, ' '),'|')
        print('| Val  Dc1: {}  '.format(eval_list1[2]).ljust(print_num, ' '), '| Val  Dc1: {}  '.format(eval_list2[2]).ljust(print_num, ' '), '| Val  Dc1: {}  '.format(eval_list3[2]).ljust(print_num, ' '),'|')
        print('| Val mJc1: {:.4f}'.format(eval_list1[1]).ljust(print_num, ' '), '| Val mJc2: {:.4f}'.format(eval_list2[1]).ljust(print_num, ' '),'| Val mJc3: {:.4f}'.format(eval_list3[1]).ljust(print_num, ' '),'|')
        print('| Val mDc1: {:.4f}'.format(eval_list1[3]).ljust(print_num, ' '),'| Val mDc2: {:.4f}'.format(eval_list2[3]).ljust(print_num, ' '), '| Val mDc3: {:.4f}'.format(eval_list3[3]).ljust(print_num, ' '), '|')
        val_m_jc1 = eval_list1[1]
        val_m_jc2 = eval_list2[1]
        val_m_jc3 = eval_list3[1]
    return eval_list1, eval_list2, eval_list3, val_m_jc1,val_m_jc2, val_m_jc3
...
def save_val_best_sup_3d(best_model, best_list, best_result,model1, model2,model3,eval_list_1,eval_list_2,eval_list_3, path_trained_model, model_name):

    if eval_list_1[2] < eval_list_2[2] and eval_list_3[2] < eval_list_2[2]:
    ...
    elif eval_list_2[2] < eval_list_1[2] and eval_list_3[2] < eval_list_1[2]:
        ...
        if best_list[2] < eval_list_3[2]:

            best_model = model1
            best_list = eval_list_3
            best_result = 'Result3'
            if best_list[2]>0.78:
                torch.save(model1.state_dict(), os.path.join(path_trained_model, 'best_{}_Dc_{:.4f}.pth'.format('result3', best_list[2])))
