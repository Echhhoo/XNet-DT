...
def save_val_best_sup_3d(num_classes, best_list, model, score_list_val, mask_list_val, eval_list, path_trained_model, path_seg_results, path_mask_results, model_name, format):
    ...
    if num_classes == 2:
        if best_list[2] < eval_list[2]:
            best_list = eval_list
            if  best_list[2]>0.78:
                torch.save(model.state_dict(), os.path.join(path_trained_model, 'best_{}_Dc_{:.4f}.pth'.format(model_name, best_list[2])))
