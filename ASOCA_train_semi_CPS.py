# nohup python ASOCA_train_semi_CPS.py >semi_CPS_fold00.out &
...
if __name__ == '__main__':
    ...
    if args.optim == 'AdamW':
        ...
        optimizer2 = torch.optim.AdamW(model1.parameters(), lr=args.lr, weight_decay=0.0001)
    elif args.optim == 'Adam':
        optimizer1 = torch.optim.Adam(model1.parameters(), lr=args.lr)
        optimizer2 = torch.optim.Adam(model1.parameters(), lr=args.lr)
    elif args.optim == 'SGD':
        optimizer1 = optim.SGD(model1.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=5*10**args.wd)
        optimizer2 = optim.SGD(model1.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=5*10**args.wd)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(num_batches['train_sup']):
            ...
            loss_train_unsup = dice_loss(pred_train_unsup1, max_train_unsup2[:].unsqueeze(1)) + dice_loss(pred_train_unsup2, max_train_unsup1[:].unsqueeze(1))
            ...
            loss_train_sup2 = dice_loss(pred_train_sup2, mask_train_sup[:].unsqueeze(1))
