...
if __name__ == '__main__':
    ...
    for i in os.listdir(path):
        ...
        # if os.path.exists(os.path.join(args.root_path+args.dataset+'/DTCL/', i)):
        #     print(f"{i} exists, skipping this iteration.")
        ...
        # 2. 局部标准差计算（滑动窗口）
        squared_hfc = LLL ** 2
        mean_hfc = uniform_filter(LLL, size=3, mode='constant')
        mean_squared_hfc = uniform_filter(squared_hfc, size=3, mode='constant')
        local_std = np.sqrt(mean_squared_hfc - mean_hfc ** 2)

        # 标准化热图到[0,1]
        hfc_heatmap = (local_std - np.min(local_std)) / (np.max(local_std) - np.min(local_std))
        hfc_heatmap = sitk.GetImageFromArray(hfc_heatmap)
        # hfc_heatmap.SetSpacing(ori_spacing)
        # hfc_heatmap = resample_image.Execute( hfc_heatmap) #执行重采样操作，将重采样后的结果重新赋值给 DTCHA
        # 可选：保存结果
        hfc_heatmap.SetSpacing(image.GetSpacing()) #设置 LLL 的像素间距为 image 的像素间距
        hfc_heatmap.SetDirection(image.GetDirection()) #设置 LLL 的方向为 image 的方向
        hfc_heatmap.SetOrigin(image.GetOrigin()) #设置 LLL 的原点为 image 的原点
        sitk.WriteImage( hfc_heatmap, 'hfc_heatmap_LL.nii.gz')
        ...
        DTCL_path=args.root_path+args.dataset+'/DTCL/'
        ...
        DTCHA_path=args.root_path+args.dataset+'/DTCHA/'
        ...
        DTCHR_path=args.root_path+args.dataset+'/DTCHR/'
