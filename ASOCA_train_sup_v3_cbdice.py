# nohup python ASOCA_train_sup_v3_cbdice.py >sup_xnetv3_cbdice_fold01.out &
import random
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--num_epochs', default=601, type=int)
    ...
    for epoch in range(args.num_epochs):
        ...
        count_iter += 1
        # lr_ = args.lr * (1.0 - epoch / args.num_epochs) ** 0.9
        ...
        for i, sampled_batch_labeled in enumerate(dataloaders['train']):
            ...
            inputs_train_4 = Variable(sampled_batch_labeled['image4'].cuda())
            ...
            with torch.amp.autocast('cuda',enabled=True):  
                outputs_train_1, outputs_train_2 ,outputs_train_3=  model(inputs_train_1, inputs_train_2, inputs_train_3, inputs_train_4)
            ...
            # loss_train_sup1_ce=ce_loss(outputs_train_1,mask_train[:].long())
                loss_train_sup1=cbdice_loss(outputs_train_1,mask_train[:].unsqueeze(1),epoch)
            ...
            # loss_train_sup2_ce=ce_loss(outputs_train_2,mask_train[:].long())
                loss_train_sup2=cbdice_loss(outputs_train_2,mask_train[:].unsqueeze(1),epoch)
            ...
            # loss_train_sup3_ce=ce_loss(outputs_train_3,mask_train[:].long())
                loss_train_sup3=cbdice_loss(outputs_train_3,mask_train[:].unsqueeze(1),epoch)
            ...
            # loss_train_unsup_ce=ce_loss(outputs_train_1,max_train2[:].long())+ce_loss(outputs_train_2,max_train1[:].long())+ce_loss(outputs_train_1,max_train3[:].long())+ce_loss(outputs_train_3,max_train1[:].long())
                ...
                # loss_train_unsup=cbdice_loss(outputs_train_1, max_train2[:].unsqueeze(1),epoch)+cbdice_loss(outputs_train_2, max_train1[:].unsqueeze(1),epoch)+cbdice_loss(outputs_train_1, max_train3[:].unsqueeze(1),epoch)+cbdice_loss(outputs_train_3, max_train1[:].unsqueeze(1),epoch)
                loss_train_unsup = dice_loss(outputs_train_1, max_train2[:].unsqueeze(1)) + dice_loss(outputs_train_2, max_train1[:].unsqueeze(1))+dice_loss(outputs_train_1, max_train3[:].unsqueeze(1)) + dice_loss(outputs_train_3, max_train1[:].unsqueeze(1))
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('sup_xnetv3_cbdice_fold03.png')
